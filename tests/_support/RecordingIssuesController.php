...
/**
 * The Issue Center controller with its flash messages captured rather than stored.
 *
 * Craft's console application refuses to hand out a session, so an action that tells the reader
 * what happened cannot be exercised in an integration run without this. Only the two methods
 * that reach for the session are replaced: the control panel check, CSRF validation, the
 * permission checks and the service underneath are all the real ones.
 */
